...
module.exports = function(db, opt, cb) {
    ...
    db.serialize(function() {
        db.run('create table if not exists markov (id1, id2, id3, '
              +'id4, id5, count, primary key (id1, id2, id3, id4, id5));');
        ...
        var insert  = db.prepare('insert or ignore into markov (id1, id2, id3, id4, id5, count) '
                                +'values (?, ?, ?, ?, ?, 0);'),
            update  = db.prepare('update markov set count = count + 1 '
                                +'where id1 = ? and id2 = ? and id3 = ? and id4 = ? and id5 = ?;');

                                
        var creative = db.prepare('select m2.id3 as id, count(*) as count from markov m1 '
        ...
        for (var n = 2; n <= 5; ++n) {
            ...
            var nxt = 'select id' + n + ' from markov where ',
                prv = 'select id' + r + ' from markov where ';
        ...
        self.creative = function m_creative(id, cb) {  
            return creative.all(id, cb);
        }
