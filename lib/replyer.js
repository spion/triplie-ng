...
module.exports = function(db, opt) {
    ...
    return function reply(text, done) {              
        ...
        function afterAssociate(err, aclusters) {
            console.log("expanded to associated", aclusters.length);
        ...
        function afterMarkov(err, answers) {
            console.log("Possible answers", answers.length);
            associate.pick(similarids, answers, afterPick);
        };
