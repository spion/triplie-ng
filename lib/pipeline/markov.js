...
function arrcompare(a1, a2) {
    return a1.filter(function(e, i) { return a2[i] == e; }).length == a1.length;
}
module.exports = function(db, opt) {

    function wordify(chain) {
        return chain.map(function(id) { return db.dict.get(id.valueOf()).word});
    }
    ...
    return function(clusters, done) {
        ...
        clusters = clusters.sort(function(c1, c2) { 
       ...
       function afterRight(err, chains, path) {
        ...
        function values(chain) { 
            return chain.map(function(id) { 
                return id.hasOwnProperty('id') ? id.id : id; 
            }) }
